...
private:
  ...
  edm::EDGetTokenT<pat::CompositeCandidateCollection> recoVertexCompositeCandidateCollection_Token_;
...
VertexCompositeNtupleProducer::VertexCompositeNtupleProducer(const edm::ParameterSet& iConfig)
{
    ...
    recoVertexCompositeCandidateCollection_Token_ = consumes<pat::CompositeCandidateCollection>(iConfig.getUntrackedParameter<edm::InputTag>("VertexCompositeCollection"));
...
VertexCompositeNtupleProducer::fillRECO(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    edm::Handle<pat::CompositeCandidateCollection> LCP3candidates;
    iEvent.getByToken(recoVertexCompositeCandidateCollection_Token_,LCP3candidates);
    const pat::CompositeCandidateCollection * LCP3candidates_ = LCP3candidates.product();
    
    edm::Handle<MVACollection> mvavalues;
    if(useAnyMVA_)
    {
      iEvent.getByToken(MVAValues_Token_,mvavalues);
      assert( (*mvavalues).size() == LCP3candidates->size() );
    }
...
//      int ntrk = cent->Ntracks();
	...
	//std::cout<<"[DEBUG] Track size="<<tracks->size()<<std::endl;
	//const reco::Track & trk = (*tracks)[it];
        const pat::PackedCandidate& trk = (*tracks)[it];
        if (!trk.bestTrack()) continue;
	...
	const reco::Track* trkPtr = trk.bestTrack();
        ...
        // Use pat::PackedCandidate for dz and dxy                                                                                  
...
//        iEvent.getByToken(tok_genParticle_,genpars);
    ...
    //RECO Candidate info
    for(unsigned it=0; it<LCP3candidates_->size(); ++it){
        
        const pat::CompositeCandidate & trk = (*LCP3candidates_)[it];
	...
	*/
	std::cout << "Number of daughters: " << trk.numberOfDaughters() << std::endl;
	...
	if (!d1) {
	  std::cout << "Failed to cast daughter 0 to pat::PackedCandidate" << std::endl;
	  return;
	}

	if (!d2) {
	  std::cout << "Failed to cast daughter 1 to pat::PackedCandidate" << std::endl;
	  return;
	}
