...
VertexCompositeNtupleProducer::fillRECO(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    edm::Handle<reco::VertexCompositeCandidateCollection> LCP3candidates;
    iEvent.getByToken(recoVertexCompositeCandidateCollection_Token_,LCP3candidates);
    const reco::VertexCompositeCandidateCollection * LCP3candidates_ = LCP3candidates.product();
...
//        iEvent.getByToken(tok_genParticle_,genpars);
    ...
    for(unsigned it=0; it<LCP3candidates_->size(); ++it){
        ...
        const reco::VertexCompositeCandidate & trk = (*LCP3candidates_)[it];
