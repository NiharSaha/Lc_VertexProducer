...
VertexCompositeSelector::fillRECO(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    edm::Handle<reco::VertexCompositeCandidateCollection> LC3Pcandidates;
    iEvent.getByToken(recoVertexCompositeCandidateCollection_Token_,LC3Pcandidates);
    const reco::VertexCompositeCandidateCollection * LC3Pcandidates_ = LC3Pcandidates.product();
    
    edm::Handle<MVACollection> mvavalues;
    if(useAnyMVA_ && useExistingMVA_)
    {
      iEvent.getByToken(MVAValues_Token_,mvavalues);
      assert( (*mvavalues).size() == LC3Pcandidates->size() );
    }
	...
	*/
	...
	//std::cout<<"[DEBUG] correct-2"<<std::endl;
	
	// Use pat::PackedCandidate for dz and dxy
	double dzvtx  = trk.dz(bestvtx);
	double dxyvtx = trk.dxy(bestvtx);
	
	// Use reco::Track for errors
    ...
    if(doGenMatching_)
    {
	    ...
	    std::cout<<"[DEBUG] correct-3"<<std::endl;
	      ...
	      std::cout<<"[DEBUG] correct-4"<<std::endl;
            }
    ...
    //RECO Candidate info
    for(unsigned it=0; it<LC3Pcandidates_->size(); ++it){
        
        const reco::VertexCompositeCandidate & trk = (*LC3Pcandidates_)[it];
	...
	//std::cout<<"[DEBUG] correct-5"<<std::endl;
